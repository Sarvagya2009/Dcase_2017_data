...
def f1_overall_framewise(O, T):
    ...
    prec = float(TP) / float(Nsys + utils.eps) #TP/ (TP+FP)
...
def f1_overall_1sec(O, T, block_size):
    if len(O.shape) == 3:#O= pred with features x classes, T=y
        O, T = utils.reshape_3Dto2D(O), utils.reshape_3Dto2D(T)
    new_size = int(np.ceil(O.shape[0] / block_size)) #divide by frame rate
